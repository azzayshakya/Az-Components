import { Button, Col, Form, Input, Row, Select, DatePicker, Spin, InputNumber, Switch } from 'antd';
...
export default function ProjectInputForm({ type, projectId, setShowInputForm, initialData }) {
  ...
  const handleEmployeeSelect = (fieldName, empName) => {
    ...
    if (emp) {
      const empIdField =
        fieldName === 'projectLeadName' ? 'projectLeadEmpId' : 'projectCoLeadEmpId';
  ...
  useEffect(() => {
    if ((isView || isEdit) && initialData) {
      ...
      if (initialData.clientCompany) {
        setCompanyOptions([
          {
            label: initialData.clientCompany,
            value: initialData.clientCompany,
          },
        ]);
      }
    } else {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      toast.error(
        err?.message ? `${err.message}, Project updation failed` : 'Project updation failed'
      );
    } finally {
      setLoading(false);
    }
                ...
                <Col md={12}>
                  <Form.Item name="clientCompany" label="Company">
                    <Select
                      ...
                      filterOption={false}
                      notFoundContent={
                        searchingCompany ? <Spin size="small" /> : 'Type to search...'
                      }
