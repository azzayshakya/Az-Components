...
//  *   - A "Permission Denied" screen is shown
//  *   - The screen remains visible for **5 seconds**
//  *   - A button is displayed:
//  *     → "Go back to Marketing Site"
//  *   - User is redirected back to:
//  *     https://www.elmech.live
 ...
 * OVERVIEW
 * --------
 * This AdminAuthProvider is responsible for handling
 * authentication and authorization for the Admin Dashboard.
 *
 * The Admin Dashboard is a **sub-website** of the main
 * marketing website (https://www.elmech.live).
 *
 * Only users with ADMIN-level roles are allowed to access
 * this application.
 *
 *
 * WEBSITE & SECURITY REQUIREMENTS
 * -------------------------------
 * - The entire application runs on **HTTPS only**
 * - Authentication is based on **cookies**
 *   (Access Token & Refresh Token)
 * - Cookies are expected to be:
 *   - HttpOnly
 *   - Secure
 *   - SameSite=Strict (or Lax depending on backend)
 *
 *
 * ENTRY FLOW (HIGH LEVEL)
 * ----------------------
 * 1. User lands on the marketing website.
 * 2. If the user is an Admin, CEO, or Manager:
 *    → They are redirected to the Admin Dashboard.
 * 3. When the Admin Dashboard loads:
 *    → This Auth Provider runs immediately.
 *
 *
 * AUTHENTICATION FLOW (ON PAGE LOAD)
 * ---------------------------------
 * 1. The application checks for authentication cookies:
 *    - Access Token
 *    - Refresh Token
 *
 * 2. An API call will be made (future implementation) to:
 ...
 *    - Admin is granted access to the dashboard
 *
 * 4. If the token is invalid or user is not authorized:
 *    - Access is denied
 *
 *
 ...
 *
 * RESPONSIBILITIES OF THIS CONTEXT
 * -------------------------------
